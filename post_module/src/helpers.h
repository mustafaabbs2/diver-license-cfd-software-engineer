...
public:
    ...
    Span(const Span<std::remove_const_t<T>>& s) noexcept : sz_(s.size()), mem_(s.data()) {}
    Span(const size_t n, T* data) noexcept : sz_(n), mem_(data) {}

    Span(const std::vector<std::remove_const_t<T>>& vec) noexcept : sz_(vec.size()), mem_(vec.data()) {}

    Span(std::vector<T>& vec) noexcept : sz_(vec.size()), mem_(vec.data()) {}

    operator std::vector<std::remove_const_t<T>>() const { return std::vector<std::remove_const_t<T>>(begin(), end()); }

    iterator begin() const { return mem_; }
    iterator end() const { return mem_ + sz_; }

    pointer data() const { return mem_; }

    bool empty() const { return sz_ == 0; }
    size_t size() const { return sz_; }

    reference operator[](const size_t idx) const { return mem_[idx]; }
...
Span<T> CastNumpyToSpan(const pybind11::array& obj) {
    ...
    if (buf.shape[0] > 0) {
        ...
        if (buf.itemsize != sizeof(T)) {
            throw std::invalid_argument("Wrong data type! Array type expeceted to be " + std::to_string(sizeof(T)) + " bytes. Type found has " + std::to_string(buf.itemsize) + " bytes");
        }
        if (buf_ndim != 1) {
